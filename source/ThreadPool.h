...
// Modified for kASA, for copyright see the Boost License 1.0, 2020 Silvio Weging
...
public:
	...
	void emplace(const function<void()>&);
	void startThreads();
    ~ThreadPool();
...
inline void ThreadPool::emplace(const function<void()>& task) {
	tasks.emplace([task]() { task(); });
	std::atomic_fetch_add_explicit(&in_flight, std::size_t(1), std::memory_order_relaxed);
}

inline void ThreadPool::startThreads() {
